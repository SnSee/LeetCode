...
int** fourSum(int* nums, int numsSize, int target, int* returnSize, int** returnColumnSizes){
    ...
    *returnSize = 0;
    ...
    for(h = 0; h < numsSize - 3; ++h){
        ...
        if(nums[h] + nums[h+1] + nums[h+2] + nums[h+3] > target){
            break;
        } 
        if(nums[h] + nums[numsSize-1] + nums[numsSize-2] + nums[numsSize-3] < target){
            continue;
        }
        for(i = h + 1; i < numsSize - 2; ++i){
            ...
            if(nums[h] + nums[i] + nums[i+1] + nums[i+2] > target){
                break;
            }
            if(nums[h] + nums[i] + nums[numsSize-1] + nums[numsSize-2] < target){
                continue;
            }
            j = i + 1;
            k = numsSize - 1;
            while(j < k){
                ...
                tempSum = nums[h] + nums[i] + nums[j] + nums[k];
                if(tempSum > target){
                    --k;
                }
